...
%	Otros modelos sobre el comportamiento reproductivo de estos parásitos también serán desarrollados. Por ultimo obtendremos una expresión para la probabilidad de apareamiento y veremos como varia por los procesos denso-dependientes que ocurren en la población de parásitos.
	...
	\section{Distribution of parasites according to the sex}\label{sec:distsexo}%Distribution of intestinal parasites according to the sex 
...
%	Por las caracter\'isticas arriba mencionadas proponemos que la distribuci\'on del n\'umero de par\'asitos hembra en un hospedador presenta una distribuci\'on stopped sums que tiene como fgp a la funci\'on $G_W \circ G_B$, donde $G_B$ es la fgp de la distribuci\'on Bernoulli ($G_B(s)=\beta + \alpha s$). % {\color{red} quien es H?}).    %$(\beta + \alpha s)$.
%	Es decir si $W$ es una variable aleatoria que representa el n\'umero de par\'asitos por hospedador %, con fgp $G$, 
%	y 	$F$ es el n\'umero de par\'asitos hembra, $F=\sum_{i=1} ^W Y_i$ con $Y_i\sim \mathrm{Ber}$, y su fgp es de la forma 

%	De manera general, sea $G_W$ la función generadora de probabilidad (fgp) de la distribución del número de parásitos en una población de hospedares. 
%	También supongamos que para cada carga parasitaria individual la fracción de todos los parásitos hembra y macho están representados por $\alpha$ y $\beta$ respectivamente, donde $\alpha+\beta=1$.
%	De esto último obtenemos que las proporciones sexuales \textit{macho} : \textit{hembra} o sex ratio de estos parásitos es $\beta / \alpha : 1$, y además si $m$ es la media de la distribución de parásitos, el número medio de parásitos hembra y macho vienen dados por $\alpha m$ y $\beta m $ respectivamente.
%	 
%	Por las características arriba mencionadas proponemos que la distribución del número de parásitos hembra en un hospedador tiene como fgp a la función $G_W \circ H$, donde $H$ es la fgp de la distribución Bernoulli ($H(s)=\beta + \alpha s$).    %$(\beta + \alpha s)$.
%	Es decir si $W$ es una variable aleatoria que representa el número de parásitos por hospedador %, con fgp $G$, 
%	y 	$F$ es el número de parásitos hembra, su fgp es de la forma 
	...
	%es el índice de dispersión de $W$. 
	%Por lo tanto si $W$ presenta sobredispersión también lo hará $F$. 
	...
	%Similarmente si $M$ es el número de parásitos macho por hospedador, su fgp viene dado por $G_M(s)=G_W(\alpha + \beta s)$.

%	Una pregunta interesante sobre estas nuevas variables $F$ y $M$ es conocer si son independientes. Fijada una distribución para $W$ una condición necesaria para la independencia de  $F$ y $M$ viene dada por la ecuación
%	\begin{equation}
%	(1-\alpha^2-\beta^2)	\sigma_W ^2 - 2 \alpha \beta \mu_W =0.
%	\end{equation} 	
%	Por lo tanto se descarta la independencia para distribuciones como la geométrica, binomial y binomial negativa. 
	
%	Una pregunta interesante sobre estas nuevas variables $F$ y $M$ es conocer si son independientes. Fijada una distribuci\'on para $W$ una condici\'on necesaria para la independencia de  $F$ y $M$ viene dada por la ecuaci\'on
%	\begin{equation}\label{indepen}
%	(1-\alpha^2-\beta^2)	\sigma_W ^2 - 2 \alpha \beta \mu_W =0.
%	\end{equation} 
%	Por lo tanto  si $F$ y $M$ se asumen independientes podemos descartar para $W$ las distribuciones  geom\'etrica, binomial y binomial negativa, ya que estas no cumplen con la condici\'on (\ref{indepen}). 
%	En la sección \ref{sec:disindep} se demuestra la independencia para la distribución de Poisson.  	
