...
for (j in (1:length(m))) {
  ...
  for (i in (1:n)) {
    ...
    output[i,4] <- output[i,3]*(exp(-sigma))^(v[i]-1);#producción de huevos
    if (output[i,2]>0) {
      output[i,5] <- output[i,3]*(exp(-sigma))^(v[i]-1);#producción de huevos fertiles
    } else output[i,5] <- 0;
...
write.table(output2, file = "simucarga.txt", sep = "\t",row.names = F, col.names = T)