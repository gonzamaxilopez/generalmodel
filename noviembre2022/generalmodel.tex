...
\usepackage{lineno}%poner numero de lineas
\linenumbers
...
\newtheorem{defn}{Definición}[subsection]
...
\end{itemize}
	...
	%{Mating probability of parasites acquired by ingestion of eggs}
	...
	%In this section we consider that the infection by parasites occurs when the host ingiere fertilized eggs of these parasites.
	...
	%The parasites responsible for this type of infection are the helminths such as \textit{Ascaris lumbricoides}, \textit{Trichuris trichiura}, among others.
	...
	%Therefore, when analyzing the variables the number of male parasites per host and the number female parasites per host, these variables cannot be independent. 
	
	%Below we will present an analysis of these variables. 
...
%	Then, the host can acquire one or more parasites, which can be male or female depending on the sexual radius of the parasite.
...
%	En lo que sigue vamos a presentar un análisis de estas variables. 
	...
	%where $\theta$
	%$\left.\frac{\partial G(s;\theta)}{\partial s}\right|_{s=s_0}=\sum_{i\geq 0}i\mathrm{Pr}(W=w;\theta)s^{i-1}$ and $G'(s;\theta)$
	where $p$ is the probability mass function of the distribution of parasites per host and $\theta$ is a parameters vector. For example, $\theta$ may include the mean, $m$, %  and variance ( v ) of the base distribution.	
	...
	%{Mean number of fertilized female parasites and a simple mating probability} % (density-independent)}
	...
	%and therefore the presence of at least one male parasite in the host ensures the fertility of all females. {\color{red}poner referencia}
...
%	\begin{prop}
%		The mean number of fertilized female parasites per host is given by
%		\begin{equation}
%			\alpha  m - \alpha G'(\alpha)
%		\end{equation}
%	\end{prop}
...
%	Notemos que si consideramos la fgp de la distribución de hembras, $G_F$, obtenemos que la ultima expresión se la puede reescribir por $G_F'(1)-\alpha G'(\alpha)$.
	...
	%the mean fertilized egg production and the mean egg production, 
	%We obtain the proportion of
	%the eggs that are fertilized by the male parasites, and therefore we obtain
	%the probability of fecundity
...
%	%Por lo tanto la probabilidad de apareamiento de una hembra, $\phi$, viene dada por la expresión
	...
	\phi(m)%=\frac{\alpha  m -\alpha  G'(\alpha)}{\alpha m}
	=1-\frac{G'(\alpha;m)}{m}
	\end{equation}
...
%	where $j\lambda(n)$ is the egg production of $j$ females and $p_n\binom{n}{j}\alpha^j\beta^{n-j}$ is the probability of having $j$ females, both cases within a host with $n$ parasites.
%	Then, we obtain the following result for the mean egg production per host 
...
%the proportion of eggs that are fertilized by male parasites. Therefore, we obtain 
...
%and therefore we obtain the probability of fecundity of the eggs or mating probability of female parasites, under the density-dependence effects, as
	...
	%{Mean effective transmission contribution per female parasite}
	...
	%[3, 4, 20], it is necessary to know the effective transmission contribution of the female population to the parasite reservoir (in the form of eggs or larvae)
	%In deterministic population models  for the  mean parasite burden such as 
	...
	\begin{equation}\label{psi}
	\psi(m)%=
	%\frac{\sum_{n\geq 0}\sum_{j=1}^{n}j\lambda(n)p_n\binom{n}{j}\alpha^j\beta^{n-j}}
	%{\sum_{n\geq 0}\sum_{j=0}^{n}jp_n\binom{n}{j}\alpha^j\beta^{n-j}}
	%= \frac{\alpha G'(z)}{\alpha G'(1)}
	=\frac{G'(z;m)}{m}   
	\end{equation}
...
%	Using the parasite host distribution we may compute the mean egg production per host as  
%	\begin{equation}
%	\lambda_0\alpha m \psi(m) 
%	\end{equation}
%	%λ0 αmψ(m, k, z) 
%	where  is the fraction of female parasites in a host 
%	
%	However, only hosts with at least one female parasite and one male parasite will effectively contribute to the parasite reservoir by the production of fertilized (or infective) eggs. Then, the mean fertilized egg production per host is (see, for example,  \citep{anderson1992infectious})
%	\begin{equation}
...
%	$$\alpha G(z)-G(\alpha z)$$
	...
	\end{equation}
	Therefore, 
	%the mean number of fertilized female parasites per host is given by the fraction
...
\section{Parasite infection by skin-penetrating}%Independence in the variables $F$ and $M$}
\label{sec:disindep}
...
\begin{figure}[h!]
	...
	\caption{The mean contribution, $\psi$, and the mating probability, $\phi$, as a function of the mean parasite burden, $m$, for a parasite population with a negative binomial distribution. 
		...
		%The black curve is the mating probability model obtained from equation (20). The red curve is the empirical mating probability obtained for the host population simulated.	
%		The mean contribution ,$\psi$, and mating probability ,$\phi$, as a function of the mean parasite burden ,$m$, for a parasite population with a negative binomial distribution. 
%	%The red point correspond case of infection by ingested egg.
%	The dots (red and blue) are the empirical value of $\psi$ and $ \phi$, obtained of simulation. 
%	The curves (continue and dash) are the theorical model of $\psi$ and $ \phi$. 
%	The red dots and continue curves correspond to infection by egg ingestion.
%	The blue dots and dash curves corresponde to infection by skin-penetrating.	
%	The black curve is the mating probability model obtained from equation \eqref{ziphi}. 
%	The red curve is the empirical mating probability obtained for the host population simulated. 
	} 
	\label{f:nb}
\end{figure}
...
\begin{figure}[h!]
	...
	%, obtained for the host population simulated. %of the simulations. 
...
%		Mating probability as a function of the mean parasite burden for a parasite population with a zero-inflated negative binomial distribution. 
%		The black curve is the mating probability model obtained from equation \eqref{ziphi}. 
%		The red curve is the empirical mating probability obtained for the host population simulated. 
	} 
	\label{f:zinb}
