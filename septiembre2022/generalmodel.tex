...
%	En lo que sigue vamos a presentar un análisis de estas variables. 
	...
	\subsection{Mean number of fertilized female parasites and a simple mating probability} % (density-independent)}
	%{Mean number of fertilized female parasites}

	The parasites treated in this work present a polygamous mating system, and therefore the presence of at least one male parasite in the host ensures the fertility of all females. {\color{red}poner referencia}
...
%	Notemos que si consideramos la fgp de la distribución de hembras, $G_F$, obtenemos que la ultima expresión se la puede reescribir por $G_F'(1)-\alpha G'(\alpha)$.
	...
	On the other hand, 
	we obtain a simple expression for the  mating probability of a female, as the quotient between the mean number of fertilized female parasites and the mean number of female parasites per host.
...
%	%Por lo tanto la probabilidad de apareamiento de una hembra, $\phi$, viene dada por la expresión
	...
	Therefore the mating probability of a female who we denoted by $\phi$ is given by
	\begin{equation}\label{probrepro1}
	\phi=\frac{\alpha  m -\alpha  G'(\alpha)}{\alpha m}=1-\frac{ G'(\alpha)}{m}
	\end{equation}
